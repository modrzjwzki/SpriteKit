...
if ( ! function_exists( 'spritekit_v3_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function spritekit_v3_posted_on() {
	...
	$posted_on = sprintf(
		...
		esc_html_x( 'Posted on %s', 'post date', 'spritekit-v3' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		/* translators: %s: post author. */
		esc_html_x( 'by %s', 'post author', 'spritekit-v3' ),
...
if ( ! function_exists( 'spritekit_v3_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function spritekit_v3_entry_footer() {
